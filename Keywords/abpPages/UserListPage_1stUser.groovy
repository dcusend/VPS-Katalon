...
public class UserListPage_1stUser {
...
//1. always select first 'Profile Owner' user whose href is not id=328618,87445. 2. to change Profile Owner to Payer. 
...
def clickEditNonPayerUser() {
    ...
    Set<String> skipIds = ["328618", "87445"] as Set    //href Ids to be skipped (logged in users)
    ...
    for (WebElement row : rows) {
        ...
        if (!editLinks.isEmpty()) {
            ...
            boolean skipUser = skipIds.any { id ->      // skip href Ids
                href.contains("id=${id}")
            }
            ...
            if (role.equalsIgnoreCase("Payer")) {      //skip Payer user
                continue
            }
...
def setDataPayerPasswordPayerConfirmPassword() {
 ...
 //1. always select first 'Payer' and user whose href is not id=328618,87445. 2. to change Payer to Profile Owner. 
  ...
  def clickEditNonProfileOwnerUser() {
    ...
    for (WebElement row : rows) {
        ...
        if (!editLinks.isEmpty()) {
            ...
            String userId = href.split("id=")[1]				// Extract user ID from href

            if (skipIds.contains(userId)) {       				//skip href id
                continue
            }

            if (role.equalsIgnoreCase("Profile Owner")) {      //skip Profile Owner user
                continue
            }
