...
// Select Fiduciary Tax from the Payment Type dropwown
		...
		WebUI.selectOptionByLabel(findTestObject('Object Repository/RAD_Pages/Landing_Page/select_NewTaxReturnAmountDue_TaxType'),'Motor Fuel Floor Tax',false)
...
// Verify the contents of Period Ending Month dropdown
		WebUI.verifyElementPresent(findTestObject(orPath_TaxTypeFilingYear + '/select_PeriodEnding_MFFT'), 30)
		WebUI.verifyElementVisible(findTestObject(orPath_TaxTypeFilingYear + '/select_PeriodEnding_MFFT'))
		
		def totalOptionsPeriodEnding = WebUI.getNumberOfTotalOption(findTestObject(orPath_TaxTypeFilingYear + '/select_PeriodEnding_MFFT'))
		WebUI.verifyEqual(totalOptionsPeriodEnding, 13)
		
		WebUI.verifyOptionsPresent(findTestObject(orPath_TaxTypeFilingYear + '/select_PeriodEnding_MFFT'),['Month','January','February','March','April','May','June'])
		WebUI.verifyOptionsPresent(findTestObject(orPath_TaxTypeFilingYear + '/select_PeriodEnding_MFFT'),['July','August','September','October','November','December'])
		
		WebElement dropdownElement = WebUiCommonHelper.findWebElement(findTestObject(orPath_TaxTypeFilingYear + '/select_PeriodEnding_MFFT'), 10)
		Select dropdown = new Select(dropdownElement)
		List<WebElement> options = dropdown.getOptions()
		List<String> optionTexts = new ArrayList<>()
		Map<String, String> optionClassByValue = new LinkedHashMap<>()
		...
		for (WebElement option : options) {
			...
			optionClassByValue.put(optionValue, optionClass)
			println("Option value: " + optionValue + " | class: " + optionClass + " | text: " + optionText)
		}
		
		String baselineFilePath = RunConfiguration.getProjectDir() + '/KatalonData/Baselines/PeriodEnding_MFFT_option_classes.json'
		File baselineFile = new File(baselineFilePath)
		Map<String, Object> baselineRaw = (Map<String, Object>) new JsonSlurper().parse(baselineFile)
		Map<String, String> baselineOptionClassByValue = new LinkedHashMap<>()
		baselineRaw.each { key, value ->
			baselineOptionClassByValue.put(String.valueOf(key), value == null ? null : String.valueOf(value))
		}
		
		List<String> mismatches = new ArrayList<>()
		baselineOptionClassByValue.each { valueKey, expectedClass ->
			if (!optionClassByValue.containsKey(valueKey)) {
				mismatches.add('Missing runtime option value: ' + valueKey)
			} else {
				String actualClass = optionClassByValue.get(valueKey)
				if ((expectedClass ?: '') != (actualClass ?: '')) {
					mismatches.add('Class mismatch for value [' + valueKey + '] expected [' + expectedClass + '] but found [' + actualClass + ']')
				}
			}
		}
		
		optionClassByValue.keySet().each { runtimeValueKey ->
			if (!baselineOptionClassByValue.containsKey(runtimeValueKey)) {
				mismatches.add('Unexpected runtime option value: ' + runtimeValueKey)
			}
		}
		
		if (mismatches.isEmpty()) {
			KeywordUtil.logInfo('Runtime option classes match baseline: ' + baselineFilePath)
		} else {
			KeywordUtil.markFailed('Baseline comparison failed:\n' + mismatches.join('\n'))
		}
...
// Verify that MD CRN is NOT visible
		WebUI.verifyElementNotVisible(findTestObject(orPath_TaxInfo + '/input_MDCRegistration'))
		
		
// Verify that Motor Fuel License Number is NOT visible
		WebUI.verifyElementNotVisible(findTestObject(orPath_TaxInfo + '/input_MotorFuelLicNum'))
		
// Verify that Motor Fuel Inspection Number is visible
		WebUI.verifyElementVisible(findTestObject(orPath_TaxInfo + '/input_MotorFuelInsNum'))
