...
for (def row = 1; row <= numOfRows; row++) {
    ...
    if (ExecuteTC.equalsIgnoreCase('Y')) {
        ...
        CustomKeywords.'iwpPages.achPersonalPaymentEntryBootstrapPage.setDataACHMain'(NameID, ACHID, EmailPhoneID, AddressID, 
            Amount, UDFID)
        ...
        if (WebUI.verifyTextPresent('Your payment plan has been successfully created', false) || WebUI.verifyTextPresent('AutoPay', false)) {
            ...
            if (WebUI.verifyTextPresent('Edits to your payment plan were completed successfully', false)) {
                ...
                if (WebUI.verifyTextPresent('Your payment plan has been cancelled successfully', false)) {
                    ...
                    CustomKeywords.'pages.WriteExcel.demoKey'(resText, datText, resColumn, datCloumn, fileLoc, nameSheet, 
                        row)
                }
            }
        } else {
            KeywordUtil.markFailed('Edits to your payment plan were completed successfully is not present on page')
		...
		KeywordUtil.markFailed("Autopay button not found. Exiting the test case,  please re-execute")
