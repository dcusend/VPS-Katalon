...
nameSheet = "CreateUserSCFNameErr"
dataFile = "QA/Bootstrap/UM-TestData/CreateUserSCFNameErr"
numOfRows = findTestData('QA/Bootstrap/UM-TestData/CreateUserSCFNameErr').getRowNumbers()
		...
		if (ExecuteTC.equalsIgnoreCase("Y"))
			{
				...
				def prefixUsername = findTestData(dataFile).getValue('Firstname', row)
				...
				WebUI.setText(findTestObject(path_AddUser + 'input_FirstName'), spUsername)
				...
				if (WebUI.verifyTextPresent('Error! Please provide the following required information as indicated below', true))
				{
					...
					KeywordUtil.markPassed("First name can not contain")
					resText = "Pass"
				...
				{
					KeywordUtil.markFailed("First name can not contain")
					resText = "Fail"
					CustomKeywords.'pages.WriteExcel.demoKey'(resText,datText,resColumn,datCloumn,fileLoc,nameSheet,row)
