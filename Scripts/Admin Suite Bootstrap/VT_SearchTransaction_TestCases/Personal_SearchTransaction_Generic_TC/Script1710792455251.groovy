...
//in each environment.  After a successful payment,
//the transaction is searched in VT.
//#################################################
...
def numOfRows, dataFile, nameSheet
	...
	nameSheet = "PersonalSearchTransactions"
	numOfRows = findTestData('QA/Bootstrap/VT-SearchTransactions-TestData/VT-PersonalSearchTransactions').getRowNumbers()
	println("Number of Records: " + numOfRows)
	dataFile = "QA/Bootstrap/VT-SearchTransactions-TestData/VT-PersonalSearchTransactions"
	...
	for (def row = 1; row <= numOfRows; row++)
		{
			...
			cardNameV = findTestData(dataFile).getValue('NameV', row)
							...
							//def sdf = WebUI.getText(findTestObject(path_Receipt + 'div_Issue Transactions'))
								...
								if (WebUI.verifyTextPresent(remID, true))
								{
									...
									// Verif static text on the page
										...
										WebUI.verifyTextPresent('Online Check', true)
