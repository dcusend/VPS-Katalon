...
for (def manualRow = 1; manualRow <= numOfRows; manualRow++)
	{
	
		ExecuteTC = dataFile.getValue('Execute', manualRow)
		System.out.println('Value of Execute is : ' + ExecuteTC)
		
		
		if (ExecuteTC.equalsIgnoreCase("Y"))
			{
				System.out.println('Begin Record Number: ' + manualRow)
					
				
				M_PaymentApp = dataFile.getValue("PaymentApp", manualRow)
				...
				println(M_Amount)
				 ...
				 M_Email = dataFile.getValue("Email", manualRow)	
				 M_Phone = dataFile.getValue("Phone", manualRow)
				 
				 M_CompanyName = RandomStringUtils.randomAlphabetic(10)
				 println(M_CompanyName)
				 
				 M_DueDate = dataFile.getValue("DueDate", manualRow)
				 println(M_DueDate)
				 M_AddressLine1 = dataFile.getValue("AddressLine1", manualRow)
				 M_AddressLine2 = dataFile.getValue("AddressLine2", manualRow)
				 M_City = dataFile.getValue("City", manualRow)
				 M_ZipCode = dataFile.getValue("ZipCode", manualRow)
				 WebUI.delay(2)
				 M_State = dataFile.getValue("State", manualRow)
				 
				 M_UDF1 = dataFile.getValue("UDF1", manualRow)
				 M_UDF2 = dataFile.getValue("UDF2", manualRow)
				 M_UDF3 = dataFile.getValue("UDF3", manualRow)
				 M_UDF4 = dataFile.getValue("UDF4", manualRow)
				 M_UDF5 = dataFile.getValue("UDF5", manualRow)
				 M_UDF6 = dataFile.getValue("UDF6", manualRow)
				 M_UDF7 = dataFile.getValue("UDF7", manualRow)
				 M_UDF8 = dataFile.getValue("UDF8", manualRow)
				 M_UDF9 = dataFile.getValue("UDF9", manualRow)
				 M_UDF10 = dataFile.getValue("UDF10", manualRow)
...
if (WebUI.verifyTextPresent(expectedMsg, false)) {
		...
		switch(executionProfile) {      //Part 2 Manage Email or Text to Pay
			
			case "QAProfile":
				
			fileLoc = "KatalonData/EmailTextToPay/Manage_EmailText_2.xlsx"
			nameSheet = "ManageEmailTextResend_CompName"
		...
		for (def searchRow = 1; searchRow <= numOfRows; searchRow++)
			{
				...
				ExecuteTC = dataFile.getValue('Execute', searchRow)
				System.out.println('Value of Execute is : ' + ExecuteTC)
		...
		//Filter (Company Name)
		CustomKeywords.'issueEmailTextToPay.Manage_EmailText_ToPay.FilterResults'(M_CompanyName)
					
		//verifying Filter results
		WebUI.verifyTextPresent('Filter applied successfully. Found 1 record(s).', false)
		
		//click on 'Resend'
		WebUI.click(findTestObject('Object Repository/AdminSuiteBootstrap_Pages/ManageEmailOrTextToPay/Resend'))
		
		//Verify Pop up
		WebUI.waitForAlert(7)
		
		String popupText = WebUI.getAlertText()
		
		assert popupText.contains('Are you sure you want to resend the payment link')
		...
		if (WebUI.verifyTextPresent("Payment link for", false) && WebUI.verifyTextPresent("has been resent successfully", false)) {
		println "Resend button worked"
		}
		else {
		println "Resend button didn't work"
		}
		
		//again applying Filter (Company Name)
		CustomKeywords.'issueEmailTextToPay.Manage_EmailText_ToPay.FilterResults'(M_CompanyName)
		
		//verify 'Status' as Active & 'Action' as Resend and Expire (enabled).
		WebUI.verifyTextPresent('Filter applied successfully. Found 1 record(s).', false)
