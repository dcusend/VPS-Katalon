...
switch(executionProfile)
{
	...
	case "Upgrade":
		...
		fileLoc = 'KatalonData/EmailTextToPay/ManualEntry_Upgrade.xlsx'
		nameSheet = 'ManualEntryError'
		dataFile = ExcelFactory.getExcelDataWithDefaultSheet('KatalonData/EmailTextToPay/ManualEntry_Upgrade.xlsx', nameSheet, true)
		...
		if (ExecuteTC.equalsIgnoreCase("Y"))
			{
				...
				// Ignore CAN from Excel and generate random 8-digit CAN, bcoz only CAN value should not be repeated
...
// First check whether alert POPUP is present
if (WebUI.waitForAlert(2)) {
	...
	// Verify alert text
	if (WebUI.verifyMatch(alertText, ErrorMessage, false)) {
	...
	// If no POPUP, check general error message on page
