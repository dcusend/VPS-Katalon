...
def parseReceipt = { String t, String r ->
    ...
    if (t && t.length() >= 55) {
        remID = t.substring(5, 35).trim()
        String s = t.substring(35, 55).trim()
        if (s) tranID = s
    }
...
Map<String, String> saleFields = [
    'name=transaction_type':'104','name=message_version':'2.0C','name=application_id':'32','name=username':'foo','name=password':'bar',
...
Map<String, String> voidFields = [
    'name=transaction_type':'106','name=message_version':'2.0C','name=application_id':'32','name=username':'foo','name=password':'bar',
