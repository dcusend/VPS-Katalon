...
def parseReceipt = { String t, String r ->
    String remID = r
    String tranID = CustomKeywords.'customClasses.LegacyVLinkScriptUtils.extractTranId'(t)
    if (t && t.length() >= 55) { remID = t.substring(5, 35).trim(); String s = t.substring(35, 55).trim(); if (s) tranID = s }
    [remID: remID, tranID: tranID]
}

Map<String, String> saleFields = [
    'name=transaction_type':'104','name=message_version':'2.0C','name=application_id':'3899','name=username':'username','name=password':'password',
    'name=response_format':'fixed_width','name=category':'RETL','name=transaction_id':'','name=approval_code':'','name=card_name':'James Smith',
    ...
    'name=country_id':'840','name=amount':'0.11','name=card_number':'','name=card_exmo':'','name=card_exyr':'','name=cvv_data':'','name=cvv_state':'',
    'name=track_data':'%B5291158864584555^AHMED/IMTIAZ ^2803201000000000023241992904000?;5291158864584555=280320123241992904?',
    'name=track_1_data':'','name=track_2_data':'','name=entry_mode':'track','name=email':'','name=order_number':'','name=tax_amount':'6.57',
...
Map<String, String> voidFields = [
    'name=transaction_type':'106','name=message_version':'2.0C','name=application_id':'3899','name=username':'username','name=password':'password',
