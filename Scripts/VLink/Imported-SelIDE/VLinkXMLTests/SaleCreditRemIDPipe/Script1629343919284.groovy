...
try {
	...
	WebUI.navigateToUrl('https://dev-algorithm.govolution.com/vlinktest/QA/version_2_0/CC_credit.html')
	CustomKeywords.'customClasses.LegacyVLinkScriptUtils.fillFields'(creditFields + ['name=remittance_id': remittanceId, 'name=transaction_id': tranID])
	CustomKeywords.'customClasses.LegacyVLinkScriptUtils.clickByLocator'('css=input[type="submit"]')
	WebUI.waitForPageLoad(pageLoadTimeoutSeconds)
	WebUI.verifyTextPresent('Transaction approved, no errors', false)
	String resTextCredit = CustomKeywords.'customClasses.LegacyVLinkScriptUtils.getTextByLocator'('css=pre')
	println(resTextCredit)
	String tranIDCredit = CustomKeywords.'customClasses.LegacyVLinkScriptUtils.extractTranId'(resTextCredit)
	println(tranIDCredit)
} finally {
