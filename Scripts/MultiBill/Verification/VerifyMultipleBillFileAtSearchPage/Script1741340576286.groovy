...
String path = fileLoc
nameSheet = "VerifyMultipleBillFileAtSearch"
	...
	if (ExecuteTC.equalsIgnoreCase("Y"))
		...
		println(rows_count)
			...
			if(rows_count>0) {
				...
				WebUI.verifyTextPresent(dataFile.getValue("PayerAddress", row), false)
				KeywordUtil.logInfo("PayerAddress present")
				WebUI.verifyTextPresent(dataFile.getValue("DueDate", row), false)
				KeywordUtil.logInfo("DueDate present")
				WebUI.verifyTextPresent(dataFile.getValue("VPayerName1", row), false)
				KeywordUtil.logInfo("PayerName present")
				WebUI.verifyTextPresent(dataFile.getValue("VPayerAddress1", row), false)
				KeywordUtil.logInfo("PayerAddress present")
				WebUI.verifyTextPresent(dataFile.getValue("VPayerName2", row), false)
				KeywordUtil.logInfo("PayerName present")
				WebUI.verifyTextPresent(dataFile.getValue("VPayerAddress2", row), false)
				KeywordUtil.logInfo("PayerAddress present")
				}
		}
}