...
String path = fileLoc
nameSheet = "VerifyStaticTextOnViewCart"
...
for (def row = 1; row <= numOfRows; row++)
{
	...
	if (ExecuteTC.equalsIgnoreCase("Y"))
		...
		/*while(WebUI.verifyElementPresent(findTestObject('Object Repository/MultiBill/MultibillSearch/btn_removeFromCartIcon'), 10)) {
		...
		//if(WebUI.verifyElementPresent(findTestObject('Object Repository/MultiBill/MultibillSearch/btn_removeFromCartIcon'), 10)) {
			...
			if(WebUI.verifyTextPresent("1 Item(s) Added to Cart",false)) {
				...
				KeywordUtil.logInfo(WebUI.getText(findTestObject('Object Repository/MultiBill/MultibillSearch/Page_Bill Lookup/div_Item(s) Added to Cart')))
				KeywordUtil.markFailed("Static Text Not Matching")
				resText = "Fail"
				CustomKeywords.'pages.WriteExcel.demoKey'(resText,datText,resColumn,datCloumn,fileLoc,nameSheet,row)
			}
		}
