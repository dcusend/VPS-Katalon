...
println("Begin Test: CardExpirationError")
...
selenium.select("name=variableField5DropdownGroupItemID", "label=Soccer")
selenium.type("name=billingName", ("Rosa Eliver").toString())
selenium.type("id=cardNumber", "4111111111111111")
selenium.type("id=spc", "123")
selenium.select("name=cardExpMonth", "label=01")
selenium.select("name=cardExpYear", "label=2023")
selenium.type("name=billingAddress", ("55 Nolan Drive").toString())
...
selenium.waitForPageToLoad("30000")
//softAssertion.assertEquals(Pattern.matches('selenium.getText("css=div.main_col")', "Invalid card expiration date.*"), true)

WebUI.verifyTextPresent(("Invalid card expiration date"), true)

println("End Test: CardExpirationError")
