...
Map<String, String> voidFields = [
	...
	'name=password': 'password',
	'name=response_format': 'fixed_width',
	'name=original_transaction_type': '103',
	'name=track_data': '',
	'name=track_1_data': '',
	'name=track_2_data': '',
	'name=tax_amount': '',
...
try {
	println('Begin Sale And Credit And Void Vantiv')
	...
	if (!saleApproved) {
		println('Sale Transaction failed, cannot Credit or Void')
		return
	}
	...
	WebUI.navigateToUrl(baseUrl + 'Production/version_2_0/CC_credit.html')
	CustomKeywords.'customClasses.LegacyVLinkScriptUtils.fillFields'(creditFields + [
	...
	boolean creditApproved = WebUI.verifyTextPresent('Transaction approved, no errors', false, FailureHandling.OPTIONAL)
	if (!creditApproved) {
		println('Credit Transaction failed, cannot Void')
		return
	}

	String creditText = CustomKeywords.'customClasses.LegacyVLinkScriptUtils.getTextByLocator'('css=pre')
	def creditReceipt = parseReceipt(creditText, GlobalVariable.VLinkSaleRemID)
	GlobalVariable.VLinkSaleRemID = creditReceipt.remID
	GlobalVariable.VLinkSaleTranxID = creditReceipt.tranID
