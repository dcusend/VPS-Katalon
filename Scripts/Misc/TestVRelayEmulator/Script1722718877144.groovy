...
def numOfRows, nameSheet, dataFileEmulator
	...
	numOfRows = findTestData(dataFileEmulator).getRowNumbers()
	...
	for (def row = 1; row <= numOfRows; row++)
		{
			...
			def Execute = findTestData(dataFileEmulator).getValue('Execute', row)
			
			if (Execute.equalsIgnoreCase("Y"))
				{
					def EmulatorDataKey = findTestData(dataFileEmulator).getValue('ID', row)
