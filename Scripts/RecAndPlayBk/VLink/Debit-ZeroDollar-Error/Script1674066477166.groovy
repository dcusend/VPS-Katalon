...
def numOfRows, dataFile, nameSheet
	...
	nameSheet = "Debit-ZeroDollar"
	numOfRows = findTestData('QA/VLinkTestData/VLinkDebitZeroDollar').getRowNumbers()
	println("Number of Records: " + numOfRows)
	dataFile = "QA/VLinkTestData/VLinkDebitZeroDollar"
	...
	for (def row = 1; row <= numOfRows; row++)
		{
			...
			if (ExecuteTC.equalsIgnoreCase("Y"))
				...
				if (WebUI.verifyTextPresent(("invalid: amount"), true))
					{
						println "invalid: amount text is present on the Confirmation page"
					...
					{
						println "invalid: amount text is not present on the Confirmation page"
